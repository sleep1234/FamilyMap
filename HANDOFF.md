...
- **部署**：VPS `www.zhp0104.fun:8090`，路径 `/vol1/1000/9自己的软件项目/家庭位置共享/`
...
- 日志：`/vol1/1000/fm.log`
- APK 下载：`/vol1/1000/app-debug.apk`

### 部署方式
使用 Python paramiko 脚本（PowerShell 的 `&` 转义有问题，不能直接 SSH）。
`.temp/` 目录下保留了 `deploy_care_fix.py`、`restart_server.py`、`upload_apk.py` 作为参考。

### 注意事项
- VPS root 分区只读，日志和数据只能写 `/vol1`
- PM2 不可用，直接用 `nohup node server.js`
- 不能 `git pull/push`，用 GitHub REST API（`.temp/push_to_github.py`）

---

## 五、下次接续注意事项

1. **验证 Bark 推送**：让用户确认 iOS 上 Bark Key 保存和推送是否正常
2. **GitHub Actions 构建失败**：需要查看 workflow 日志，可能是新文件导致编译错误
3. **本地缓存验证**：让用户测试启动速度是否改善
4. **头像缓存验证**：重启 App 后头像是否从本地加载
5. **多设备登录退出**：之前有相关会话但未完全确认