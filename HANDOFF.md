...
## 三-C、今日会话进度（2026-06-28）

### 已完成
| 问题 | 修复方案 | 文件 |
|------|----------|------|
| iOS 安装后闪退 | Info.plist 包含无效 key（UIUserNotificationSettings、UIBackgroundMode-location、UIApplicationSceneManifest）导致 plist 解析失败。清理无效 key | `ios/Runner/Info.plist` |
| iOS 无 Podfile | 大量原生插件需要 CocoaPods 依赖，缺少 Podfile 导致原生插件无法注册。新建 `ios/Podfile` | `ios/Podfile` |
| iOS AppDelegate 公关问题 | `FlutterImplicitEngineDelegate` 模式不兼容，改为标准 `GeneratedPluginRegistrant.register(with: self)` | `ios/Runner/AppDelegate.swift` |
| iOS 登录后闪退 | `activity_recognition_flutter` 调用 CoreMotion 时缺少 `NSMotionUsageDescription` 声明，导致崩溃。添加权限描述 | `ios/Runner/Info.plist` |
| iOS 通知不显示 | `DarwinInitializationSettings` 中 `requestAlertPermission: false`，`requestPermission()` 在 iOS 上直接返回 true 不做实际检查。改为 true + `IOSFlutterLocalNotificationsPlugin` 显式请求权限 | `lib/services/notification_service.dart` |
| Bark 推送超时阻塞 | HTTP 请求超时 10秒过长，串行逐个用户推送。降低到 5秒 | `services/bark.js` |

### 服务端已重启
- `pm2 restart familymap` 生效

### 待验证
- [ ] iOS 安装后正常登录，不闪退
- [ ] iOS 通知权限弹窗并正常显示本地通知
- [ ] Android/iOS 互发消息无明显延迟
- [ ] Bark 推送不再超时
---
...
1. **验证 iOS 安装不闪退**：用户已确认正常登录，通知权限弹窗已修复
2. **验证消息通知无延迟**：Android/iOS 互发消息流畅性
3. **验证 Bark 推送**：后台通知不再超时（已降低到 5秒）
4. **多设备登录退出**：已实现 force_logout + connect_error 双重检测
5. **Bark Key 配置**：当前仅台州古巨基(iOS) 配置了 bark_key，其他用户需在设置中配置